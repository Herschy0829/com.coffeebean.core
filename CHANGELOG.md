...
### Changed
- **内置 registry 指向 excel v0.5.2**：excel 又修了两个**把生成产物真丢进工程编译**才暴露的问题 ——
  含字典列（`_kv`）的表生成的代码少了 `using System.Collections.Generic;`（编译不过），
  以及两个列名去后缀后撞成同一字段名（`State_e` + `State_ea`）会在生成代码里变成 CS0102/CS0101。
  指针：core → v0.1.70、excel → v0.5.2。

### Note（流程教训）
excel 这轮连着出了 0.5.0 / 0.5.1 / 0.5.2 三个版本，原因是**先发版、后拿真实项目 dogfood**。
正确的顺序是：真实表 dry-run + 生成产物真编译 → 一起修完 → 一次发版。
后续模块改动按这个顺序走。

### Tests
- 全量 EditMode **818**（817 通过 + 1 有意跳过）。

## [0.1.69] - 2026-09-18

### Changed
- **内置 registry 指向 excel v0.5.1**：拿真实项目的 28 张配置表跑了 dry-run 校验，
  修掉两个框架窟窿（数组分隔符少了 `_` —— 真实表里数组都写成 `13_100`；
  说明行/图例行主键为空会把整张表打挂 —— 现在自动跳过并警告列行号）。
  指针：core → v0.1.69、excel → v0.5.1。
- 修 `ModuleMarker` 版本号没跟上 `package.json`（0.1.67 → 0.1.69）——
  `ModuleManagerVersionTests` 逮到的，这正是它存在的意义。

### Tests
- 全量 EditMode **816**（814 通过 + 1 有意跳过）。
...
### Changed
- **内置 registry 指向 asset v0.6.1**：修掉 v0.6.0 里"未解析 Profile 变量"整条检查的假阳性
  （`[...]` 在 Addressables 里是正常语法：引用变量 / 内联 C# 表达式；且未定义变量会被**静默吃掉**，
  所以"残留方括号"这个判据从根上不成立）。指针：core → v0.1.64、asset → v0.6.1。

### Tests
- 全量 EditMode **719**（718 通过 + 1 有意跳过）。
